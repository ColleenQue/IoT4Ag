...
class Direct:
    ...
    def model_states_callback(self, msg):    
        ...
        if self.current_position_x < self.current_goal_x+0.1 and self.current_position_x > self.current_goal_x-1:
            if self.current_position_y < self.current_goal_y+0.1 and self.current_position_y > self.current_goal_y-1:
                self.publish_goal()
        self.publish_goal()
    ...
    def publish_goal(self):
        ...
        cmd_goal.header.stamp = rospy.Time.now()
        cmd_goal.header.frame_id = 'map'


        self.current_goal_x = cmd_goal.pose.position.x = -5
        self.current_goal_y = cmd_goal.pose.position.y = -5
        cmd_goal.pose.orientation.w=1 
        print(self.current_index)
        print(cmd_goal.pose.position.x)
