...
class Direct:
    ...
    def model_states_callback(self, msg):    
        ...
        #for graphing
        self.trajectory_x.append(self.current_position_x)
        self.trajectory_y.append(self.current_position_y)
    ...
    def area_callback(self,msg):
        ...
        if len(contour_area_list)>0:
            ...
            # if self.around or self.area >= 20000:
            #     self.around = True
            ...
            if self.area>=6000:
                #self.around = True
                #create circular way points
                #self.move_around()
                delta_x = math.cos(self.current_angle)
                delta_y = math.sin(self.current_angle)

                mark_x = self.current_position_x + delta_x
                mark_y = self.current_position_y + delta_y
                ...
                for x,y in self.store_targets:
                    ...
                    if self.Pythagorean(x_diff,y_diff)<3:
                        exists = True
                        break
                       
                if not exists:
                    self.store_targets.append([mark_x,mark_y])
            ...
            self.around = False
            if self.glitching or (self.area>=400 and self.area<=6000):
    ...
    def move_around(self):
        #the ball currently close to a red ball --> move around
        #construction
    ...
    def perform_action(self):
        ...
        distance = self.Pythagorean(self.distance_x,self.distance_y)
    ...
    def signal_handler(self,signal,frame):
        # plotting the points 
        plt.plot(node.trajectory_x, node.trajectory_y)
        plt.show()
        sys.exit(0)
