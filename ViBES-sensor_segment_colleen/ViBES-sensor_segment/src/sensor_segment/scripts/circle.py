...
def publish_waypoints():
    ...
    waypoints = circular_motion(0,0,7)
    print(waypoints)
...
def create_waypoint(x, y,qx=0,qy=0,qz=0,qw=1):
...
def circular_motion(start_x,start_y,stopping_points):
    print("hello")
    goal_angle = math.pi/stopping_points
    around_points = [[start_x,start_y]]
    i=0
    curr_x,curr_y = start_x,start_y
    while i<stopping_points:
        delta_x = math.cos(goal_angle) *2
        delta_y = math.sin(goal_angle) *2
        curr_x = curr_x+delta_x
        curr_y = curr_y + delta_y
        around_points.append([curr_x,curr_y])
        goal_angle -= math.pi/stopping_points *2
        i+=1
